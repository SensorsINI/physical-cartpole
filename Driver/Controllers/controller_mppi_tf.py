...
class controller_mppi_tf(template_controller):
    ...
    def trajectory_rollouts(self, s, u, delta_u, target_position):
        ...
        if predictor_type != 'EulerTF':
            ...
            Q = u + delta_u
            s = predictor.predict_tf(tf.convert_to_tensor(s, dtype=tf.float32), tf.convert_to_tensor(Q, dtype=tf.float32)).numpy()
            total_cost, dd, ep, ekp, ekc, cc, ccrc = self.stage_cost(s, u, delta_u, target_position)
    ...
    def step(self, s, target_position, time=None):
        ...
        if self.logging:
            ...
            self.logs.get("realized_costs").append(np.copy(self.stage_cost(self.state, self.u[0], self.u_prev[1] - self.u[0], target_position)))
