...
import platform
import subprocess
SERIAL_PORT = subprocess.run(['ls /dev/tty.usbserial*'], shell=True, stdout=subprocess.PIPE, universal_newlines=True).stdout[:-1] if platform.system() == 'Darwin' else '/dev/ttyUSB0'
