...
entity quadrature_encoder_ENCODER_AXI_s_axi is
generic (
...
end entity quadrature_encoder_ENCODER_AXI_s_axi;
...
architecture behave of quadrature_encoder_ENCODER_AXI_s_axi is
    type states is (wridle, wrdata, wrresp, wrreset, rdidle, rddata, rdreset);  -- read and write fsm states
    signal wstate  : states := wrreset;
    signal rstate  : states := rdreset;
    signal wnext, rnext: states;
    constant ADDR_RESET_DATA_0 : INTEGER := 16#10#;
    constant ADDR_RESET_CTRL   : INTEGER := 16#14#;
    constant ADDR_COUNT_DATA_0 : INTEGER := 16#18#;
    constant ADDR_COUNT_CTRL   : INTEGER := 16#1c#;
    ...
    signal int_count_ap_vld    : STD_LOGIC;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_COUNT_CTRL =>
                        rdata_data <= (0 => int_count_ap_vld, others => '0');
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
                end if;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (count_ap_vld = '1') then
                    int_count <= UNSIGNED(count); -- clear on read
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (count_ap_vld = '1') then
                    int_count_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_COUNT_CTRL) then
                    int_count_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
