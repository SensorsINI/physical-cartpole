...
typedef struct {
    ...
    XSecloc_shell_uio_map maps[ MAX_UIO_MAPS ];
} XSecloc_shell_uio_info;

/***************** Variable Definitions **************************************/
static XSecloc_shell_uio_info uio_info;
...
static int uio_info_read_name(XSecloc_shell_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSecloc_shell_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSecloc_shell_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSecloc_shell_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSecloc_shell_Initialize(XSecloc_shell *InstancePtr, const char* InstanceName) {
	XSecloc_shell_uio_info *InfoPtr = &uio_info;
