----------------------------------------------------------------
--          Top module for controller with AXI Stream          --
--  This module has AXI Stream interface for input and output. --
--  Input AXI Stream is translated to the controller format.   --
--  Output controller format is translated to AXI Stream.      -- 
----------------------------------------------------------------
--
-- =====================================================================
...
--
-- Then update line ~161 in the architecture to match.
-- =====================================================================
library ieee;
...
architecture RTL of controller_axis is
    ...
    signal ap_ready_r      : std_logic;
    signal ap_ready        : std_logic;
    signal core_in_valid   : std_logic;
    signal core_in_data_r  : std_logic_vector(TOTAL_INPUT_BITS - 1 downto 0);
    signal core_in_data    : std_logic_vector(TOTAL_INPUT_BITS - 1 downto 0);
    ...
    signal read_inputs_r   : integer := 0;
    signal read_inputs     : integer := 0;
    signal write_outputs_r : integer := 0;
    signal write_outputs   : integer := 0;
...
begin
    ...
    b_conv : process(cs, S_AXIS_TVALID, S_AXIS_TLAST, S_AXIS_TDATA, core_in_data_r, 
                     read_inputs_r, write_outputs_r, ap_done, ap_ready_r, core_out_data, M_AXIS_TREADY)
    begin
        ns              <= cs;
        ...
        case cs is
            ...
            when read_axi_inputs =>
                if S_AXIS_TVALID = '1' then
                    ...
                    core_in_data((BITS_PER_INPUT * read_inputs_r) + BITS_PER_INPUT - 1 downto (BITS_PER_INPUT * read_inputs_r)) 
                        <= S_AXIS_TDATA(BITS_PER_INPUT - 1 downto 0);
                    read_inputs <= read_inputs_r + 1;
                    -- Check if it is the last input data
                    if S_AXIS_TLAST = '1' then
                        -- Move to feed_controller state
            ...
            when wait_controller_ready =>
                if ap_ready_r = '0' then
                    -- Remain ap_start until ap_ready = '1'
                    ap_start <= '1';
                else
            ...
            when write_axi_outputs =>
                if M_AXIS_TREADY = '1' then
                    ...
                    M_AXIS_TDATA(BITS_PER_OUTPUT - 1 downto 0) 
                        <= core_out_data((BITS_PER_OUTPUT * write_outputs_r) + BITS_PER_OUTPUT - 1 downto (BITS_PER_OUTPUT * write_outputs_r));
    ...
    -- ap_idle is derived from adapter's ready signal
    ap_idle <= ap_ready;
