...
unsigned short POLLING_PERIOD_MS				=		10;  /* CONTROL_Init / show mux replace this; the PC cannot overwrite period while the mux is on. */
unsigned short CONTROL_SLOWDOWN						=		0;
bool CONTROL_SYNC									=		true;

// Calculating derivatives and dead angle detection
unsigned short TIMESTEPS_FOR_DERIVATIVE				=		2;  // LSTM / long pole: 2 × 10 ms. IROS short pole chip uses 10 × 1 ms.
...
float SliderTargetHalfLength						=		0.12;  // JB pot rails: ±12 cm for every controller
...
int MOTOR_PWM_PERIOD_IN_CLOCK_CYCLES				=		7200;
int MOTOR_FULL_SCALE_SAFE							=		6840;
...
/* Show mux overwrites this with the selected controller profile. */
...
int MOTOR_PWM_PERIOD_IN_CLOCK_CYCLES				=		10000;
int MOTOR_FULL_SCALE_SAFE							=		9500;
