...
def _run_gate_polls(gate, angle, positions, target_position=0.0, dt=0.025):
    gate.set_time_quantum(dt)
    s = create_cartpole_state()
    s[ANGLE_IDX] = angle
    time = 0.0
    for position in positions:
        s[POSITION_IDX] = position
        gate.should_sample(s, target_position, time=time)
        time += dt
...
def test_secloc_status_counts_both_changed_skips():
    ...
    for angle, position in zip(angles, positions):
        s[ANGLE_IDX] = angle
        s[POSITION_IDX] = position
        gate.should_sample(s, 0.0, time=time)
        time += dt
...
def test_peek_should_sample_does_not_mutate_gate_state():
    ...
    gate.set_time_quantum(0.02)
    s = create_cartpole_state()
    s[ANGLE_IDX] = 0.02
    s[POSITION_IDX] = 0.01

    gate.logic.time_last = 0.0
    gate.logic.tick_last = 0
    ...
    would_update = gate.peek_would_update(s, 0.0, time=0.02)
    ...
    spike = gate.should_sample(s, 0.0, time=0.02)
